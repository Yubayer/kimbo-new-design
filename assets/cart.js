  ...
  updateQuantity(line, quantity, name, variantId) {
    ...
    fetch(`${routes.cart_change_url}`, { ...fetchConfig(), ...{ body } })
      ...
      .then((state) => {
        ...
        let mainCartSectionIdFooter = this.getSectionsToRender().find(section => section.id === 'main-cart-footer')
        let mainCartSectionIdItems = this.getSectionsToRender().find(section => section.id === 'main-cart-items')


        let textContent = parsedState.sections[mainCartSectionIdFooter.section]
        let footerhtml = new DOMParser().parseFromString(textContent, 'text/html')

        if(document.querySelector(".cart--totals")) document.querySelector(".cart--totals").innerHTML = footerhtml.querySelector(".cart--totals").innerHTML
        
        let textContentItems = parsedState.sections[mainCartSectionIdItems.section]
        let itmsHtml = new DOMParser().parseFromString(textContentItems, 'text/html')

        if(itmsHtml.querySelector(".cart-items")) {
          if(document.querySelector(".cart-items")) document.querySelector(".cart-items").innerHTML = itmsHtml.querySelector(".cart-items").innerHTML
        }
        ...
        console.log(updatedCartTotal+"Cart 11");
